...
for sample_text in sample_text_i:
    ...
    if "not" in sample_text:
         if labelencode.inverse_transform(prediction_SVM)[0] =='__label__2':
             pos.append(1)
         else:
             pos.append(0)
    else:
         if labelencode.inverse_transform(prediction_SVM)[0] == '__label__1':
             ...
             neg.append(0)
score = SentimentIntensityAnalyzer().polarity_scores(stri)
if score['neg'] > score['pos']:
  
  if score['neg']>0 and score['neg']<=0.3:
    send.append(["negative"])
  elif score['neg']>0.3 and score['neg'] <=0.6:
    send.append(["moderately negative"])
  else:
    send.append(["highly negative"])
elif score['neg'] < score['pos']:

  if score['pos']>0 and score['pos']<=0.3:
    send.append(["positive"])
  elif score['pos']>0.3 and score['pos'] <=0.6:
    send.append(["moderately positive"])
  else:
    send.append(["highly positive"])



else:
  send.append(["Neutral Sentiment"])
send.append(pos)
send.append(neg)
